...
class Student(ModelTimeTracking):
    ...
    surname = models.CharField(
        verbose_name="Surname", max_length=30)
    middle_name = models.CharField(
        verbose_name="Middle Name", max_length=30)
    ...
    passport_series = models.CharField(
        verbose_name="Passport Series", max_length=2)
    passport_number = models.CharField(
        verbose_name="Passport Number", max_length=7)
    ...
    major = models.ForeignKey(to="students.Major",
                              verbose_name="Major", null=True, on_delete=models.SET_NULL)
    ...
    discount_to = models.DateField(
        verbose_name="Discount To", blank=True, null=True)
    ...
    passport_document = models.FileField(verbose_name="Passport Document", upload_to=FilePaths.get_passport_document_path,
                                         validators=[FileExtensionValidator(allowed_extensions=['pdf', 'heif', 'hevc'])])
    IELTS_document = models.FileField(verbose_name="IELTS Document", upload_to=FilePaths.get_IELTS_document_path,
                                      validators=[FileExtensionValidator(allowed_extensions=['pdf', 'heif', 'hevc'])])

    contract_document = models.FileField(
        verbose_name="Contract Document", upload_to=FilePaths.get_contract_document_path, blank=True, null=True)
    status = models.PositiveSmallIntegerField(
        verbose_name="Status", choices=Statues.STATUS_CHOICES, default=Statues.RECENTLY_ADDED, blank=True)
    ...
    def get_statistics():
        return Student.objects.aggregate(
            ...
            approved_students=Count('id', filter=Q(
                status=Statues.ACCEPTED), distinct=True),
            recently_added=Count('id', filter=Q(
                status=Statues.RECENTLY_ADDED), distinct=True),
...
class Comment(ModelTimeTracking):
    author = models.ForeignKey(
        verbose_name="Author", to="users.AdminUser", on_delete=models.SET_NULL, null=True)
